...
(() => {
  ...
  // Convert a Date (any tz) to a "wall clock" string in Nepal time (UTC+5:45)
  function toNPT(date){
    const utcMs = date.getTime() + date.getTimezoneOffset() * 60000; // -> true UTC ms
    const npt = new Date(utcMs + NPT_OFFSET_MIN * 60000);
    return npt;
  }
  ...
  async function loadFixtures(){
    ...
    try{
      ...
      if (cached){
        ...
        statusRegion.textContent = `Could not load fixtures (${err.message}). Please check your connection and reload.`;
        fixturesList.innerHTML = `<div class="empty-state">No fixture data available offline.</div>`;
      }
    }
  }
